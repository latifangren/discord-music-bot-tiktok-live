...
import { GoogleVideoExtractor } from "./extractors/index.js";
...
class MusicBot implements IMusicBot {
  ...
  constructor() {
    ...
    Logger.debug(
      `MusicBot: Player configured with volume ${config.player.volume}, selfDeaf: ${config.player.selfDeaf}`,
    );
    this.init();
  }
  ...
  async init() {
    ...
    // uncomment this if you had vps with good ip reputation, google video more stable
    Logger.debug("MusicBot: Registering GoogleVideo extractor...");
    await this.player.extractors.register(GoogleVideoExtractor, {});
    await sleep(1000);

    const extractors = Array.from(this.player.extractors.store.keys());
    console.log(
      chalk.yellow("Extractors registered:\n" + extractors.join("\n")),
    );
    Logger.debug(
      `MusicBot: Extractor registration complete - ${extractors.length} extractors loaded`,
    );
    await sleep(1000);
    ...
    Logger.success(
      `Bot is ready! Serving ${this.client.guilds.cache.size} server`,
    );
    Logger.debug(
      `MusicBot: Bot ready - serving ${this.client.guilds.cache.size} guilds`,
    );

    if (config.bot.activity.name && this.client.user) {
      Logger.debug(
        `MusicBot: Setting bot activity to "${config.bot.activity.name}" with type ${config.bot.activity.type}`,
      );
  ...
  async loadCommands() {
    ...
    Logger.debug(
      `MusicBot: Found ${commandFiles.length} command files in ${commandsPath}`,
    );
    ...
    Logger.debug(
      `MusicBot: Command loading complete - ${this.commands.size} commands registered`,
    );
  }
